...
@Component
public class SpringDatabaseInitializer {

    private final DataSource dataSource;

    public SpringDatabaseInitializer(DataSource dataSource) {
    ...
    private void create(Connection con) throws SQLException {
        String sql = "drop table if exists data_springboot;\n" + "create table data_springboot(\n" + "    id bigint not null primary key,\n"
    ...
    private void insert(Connection con, int dataCount) throws SQLException {
        String sql = "insert into data_springboot(id, " + IntStream.range(1, 10).mapToObj(i -> "value_" + i).collect(Collectors.joining(", "))
